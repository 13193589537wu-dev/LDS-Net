...
class Topformer(BaseModule):
    ...
    # 修改前：def init_weights(self):
    # 修改后：增加 pretrained 参数，默认为 None
    def init_weights(self, pretrained=None):
        # 1. 优先使用传入的参数，如果没有传入，则检查对象是否有 pretrained 属性
        if pretrained is not None:
            self.pretrained = pretrained
        
        # 2. 如果 self 没有 pretrained 属性（即 init_cfg 为空的情况），初始化它为 None
        ...
        # 3. 现在的 self.pretrained 一定存在了，不会报 AttributeError
        if isinstance(self.pretrained, str):
            logger = get_root_logger()
            checkpoint = _load_checkpoint(self.pretrained, logger=logger, map_location='cpu')
            if 'state_dict_ema' in checkpoint:
