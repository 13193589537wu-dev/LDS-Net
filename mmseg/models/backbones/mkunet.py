...
# 导入 MMSegmentation 注册器
from ..builder import BACKBONES

import torch.nn.init as init

# 兼容 PyTorch 原生的 trunc_normal_
try:
    from torch.nn.init import trunc_normal_ as trunc_normal_tf_
except ImportError:
    # 如果 PyTorch 版本太老没有 trunc_normal_，退化为普通正态分布
    def trunc_normal_tf_(tensor, mean=0., std=1., a=-2., b=2.):
        return init.normal_(tensor, mean=mean, std=std)

# 手写一个 named_apply 替代 timm 的版本
def named_apply(fn, module, name=''):
    fn(module, name)
    for child_name, child in module.named_children():
        named_apply(fn, child, name + '.' + child_name if name else child_name)
#--------------------------------------------------------------------------------------
__all__ = ['MK_UNet_T', 'MK_UNet_S', 'MK_UNet']
...
class MK_UNet_T(nn.Module):
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(pretrained, str):
            # 如果你有预训练权重需求，可以在这里调用 mmcv 的负载函数
            from mmcv.cnn.utils import load_checkpoint
            from mmcv.utils import get_logger
            logger = get_logger('mmseg')
            load_checkpoint(self, pretrained, strict=False, logger=logger)
        elif pretrained is None:
            # 实际上你在 __init__ 里的每个子模块都已经 init 过一次了
            # 如果想确保万无一失，可以在这里再次执行一遍全局初始化逻辑（可选）
            pass
        else:
...
class MK_UNet_S(MK_UNet_T):
    def __init__(self, num_classes=1, in_channels=3, channels=[8,16,32,48,80], depths=[1, 1, 1, 1, 1], kernel_sizes=[1,3,5], expansion_factor=2, gag_kernel=3, **kwargs):
        super().__init__(num_classes, in_channels, channels, depths, kernel_sizes, expansion_factor, gag_kernel, **kwargs)


@BACKBONES.register_module()
class MK_UNet(MK_UNet_T):
