_base_ = [
    '../_base_/models/deeplabv3plus_r50-d8.py',   # 使用官方 DeepLabV3+ base
...
model = dict(
    type='EncoderDecoder',
    pretrained=None,   #'open-mmlab://resnet50_v1c',  # 可换成 None，如果不使用预训练
    backbone=dict(
        type='ResNet',
        depth=50,
        num_stages=4,
        out_indices=(0, 1, 2, 3),
        dilations=(1, 1, 2, 4),   # ASPP 需要大膨胀率
        strides=(1, 2, 1, 1),
        norm_cfg=norm_cfg,
        norm_eval=False,
        style='pytorch'
...
work_dir = './work_dirs/qtpl/deeplabv3plus'
